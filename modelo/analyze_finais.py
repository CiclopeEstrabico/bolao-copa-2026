...
analyze_finais.py
=================
Script 8 — Copa 2026 Modelling Pipeline  (v2 — K-att / K-def)
Analisa a Final e a disputa de Terceiro Lugar com o modelo Dixon-Coles/GRU.

Jogos das Finais usando apenas os nomes em inglês.
Linha 1: Final (Fator de pontos: 2.0)
Linha 2: Terceiro Lugar (Fator de pontos: 1.8)
...
HEATMAP_DIR = os.path.join(RESULTS_DIR, "heatmaps_finais")
...
#   Linha 1: "Time A, Time B" (Final)
#   Linha 2: "Time A, Time B" (3º Lugar)
#   "Time A, Time B, golsA, golsB" -> Jogo finalizado (ex: "Brazil, Japan, 2, 1")
# Use "TBD" para adversários ainda indefinidos.
# ─────────────────────────────────────────────────────────────────────
RAW_CONFRONTOS = [
    "TBD, TBD",  # Final (Fase 'final', fator 2.0)
    "TBD, TBD",  # 3rd Place (Fase 'terceiro', fator 1.8)
]
...
# ─────────────────────────────────────────────────────────────────────
def compute_expected_points(M, fase_key="final"):
    fatores = {
        'grupos': 1.0,
        '16avos': 1.2,
        'oitavas': 1.4,
...
def main():
    print("=" * 65)
    print("  analyze_finais.py — Copa 2026 Finais (Final & 3º Lugar)  ")
    print("=" * 65)
    ...
    print("\n[3/4] Analisando Final & Terceiro Lugar...")
    rows = []

    for idx, line in enumerate(RAW_CONFRONTOS, 1):
        parts = [p.strip() for p in line.split(',') if p.strip()]
        if len(parts) < 2:
            print(f"  [{idx:>2}/2] Linha inválida: {line}")
            continue
        ...
        if idx == 1:
            ...
            fase_key = "terceiro"
            match_name = "3rd Place"

        if not ta_known and not tb_known:
            print(f"  [{idx:>2}/2] {match_name}: {ta} vs {tb}  → Ambos TBD, pulando")
            continue

        if not ta_known:
            print(f"  [{idx:>2}/2] {match_name}: {ta} TBD — usando ELO/K padrão")
            elo_a = 1500.0
            K_att_a = K_def_a = 1.0
        else:
            elo_a   = get_elo(ta)
        ...
        if not tb_known:
            print(f"  [{idx:>2}/2] {match_name}: {tb} TBD — usando ELO/K padrão")
            elo_b = 1500.0
            K_att_b = K_def_b = 1.0
        else:
            elo_b   = get_elo(tb)
        ...
        EV = compute_expected_points(M, fase_key)
        ...
        print(f"  [{idx:>2}/2] {match_name}  {ta} vs {tb}{status}")
        ...
        fname  = f"Finals_{match_name.replace(' ', '_')}_{safe_a}_vs_{safe_b}.png"
        save_heatmap(M, EV, ta, tb, match_name,
    ...
    out_csv = os.path.join(RESULTS_DIR, "finais_analysis.csv")
    df.to_csv(out_csv, index=False)
    print(f"    >> {out_csv}")

    if not df.empty:
        print("\n" + "=" * 72)
        print(f"  {'Jogo':<12}  {'Home':<18}  {'Away':<18}  "
        ...
        for _, r in df.iterrows():
            ...
            print(f"  {r['id']:<12}  {r['home']:<18}  {r['away']:<18}  "
