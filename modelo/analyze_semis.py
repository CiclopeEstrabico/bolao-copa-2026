...
analyze_semis.py
================
Script 7 — Copa 2026 Modelling Pipeline  (v2 — K-att / K-def)
Analisa os 2 jogos das semifinais com o modelo Dixon-Coles/GRU.

Chaveamento oficial das semifinais usando apenas os nomes em inglês.
...
HEATMAP_DIR = os.path.join(RESULTS_DIR, "heatmaps_semis")
...
# ─────────────────────────────────────────────────────────────────────
def compute_expected_points(M, fase_key="semis"):
    fatores = {
        'grupos': 1.0,
        '16avos': 1.2,
        'oitavas': 1.4,
...
def main():
    print("=" * 65)
    print("  analyze_semis.py — Copa 2026 Semifinais  ")
    print("=" * 65)
    ...
    print("\n[3/4] Analisando 2 jogos das semifinais...")
    rows = []
    ...
    for idx, line in enumerate(RAW_CONFRONTOS, 1):
        ...
        if not ta_known and not tb_known:
            print(f"  [{idx:>2}/2] SF_{idx}: {ta} vs {tb}  → Ambos TBD, pulando")
            continue

        if not ta_known:
            print(f"  [{idx:>2}/2] SF_{idx}: {ta} TBD — usando ELO/K padrão")
            elo_a = 1500.0
            K_att_a = K_def_a = 1.0
        else:
            elo_a   = get_elo(ta)
        ...
        if not tb_known:
            print(f"  [{idx:>2}/2] SF_{idx}: {tb} TBD — usando ELO/K padrão")
            elo_b = 1500.0
            K_att_b = K_def_b = 1.0
        else:
            elo_b   = get_elo(tb)
        ...
        EV = compute_expected_points(M, "semis")
        ...
        print(f"  [{idx:>2}/2] SF_{idx}  {ta} vs {tb}{status}")
        ...
        fname  = f"SF_{idx}_{safe_a}_vs_{safe_b}.png"
        save_heatmap(M, EV, ta, tb, f"SF_{idx}",
    ...
    out_csv = os.path.join(RESULTS_DIR, "semis_analysis.csv")
