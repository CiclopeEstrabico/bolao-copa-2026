...
window.renderEstatisticas = function () {
  ...
  h += _dCard("🏆", "Líder", melhorPts?.participante.apelido || melhorPts?.participante.nome || "—", melhorPts ? melhorPts.stats.total.toFixed(1) + " pts" : "—", "var(--dourado)", "Quem tem mais pontos no total. Cada jogo vale mais dependendo da fase: grupos, oitavas, quartas, semi e final têm multiplicadores crescentes.");
  h += _dCard("🔮", "Vidente", melhorRes?.participante.apelido || melhorRes?.participante.nome || "—", melhorRes ? melhorRes.stats.acertos_resultado + " acertos de resultados" : "—", "#86efac", "Quem mais acertou o desfecho dos jogos — vitória do time da casa, empate ou vitória do visitante — sem precisar acertar o placar exato.");
  h += _dCard("🎯", "Atirador de Elite", melhorExato?.participante.apelido || melhorExato?.participante.nome || "—", melhorExato ? melhorExatoCount + " placares exatos" : "—", "var(--verde-ok)", "Quem mais acertou o placar exato. Placares normais dão +3 pts de bônus; placares com 4 ou mais gols no total dão +5 pts.");
  h += _dCard("🦓", "Zebra de Ouro", melhorZebra?.apelido || melhorZebra?.nome || "—", melhorZebra ? zebraCount + " zebras domadas" : "—", "#fcd34d", "Quem mais acertou resultados que menos de 20% do grupo havia apostado — palpites raros E corretos. Coragem com precisão.");
  h += _dCard("💎", "Mestre dos Bônus", mestreBonus?.participante.apelido || mestreBonus?.participante.nome || "—", mestreBonus ? (mestreBonus.jogosComBonus ?? 0) + " jogos com bônus" : "—", "#c084fc", "Quem somou mais jogos com algum bônus: placar exato (+3 ou +5 pts), diferença de gols correta (+1 pt) ou gols de um time corretos (+1 pt).");
  ...
  h += _dCard("🎲", "Anarquista", (anarqApo?.apelido || anarqApo?.nome || "—"), (anarqApo ? "Δ" + anarqMedia + " de distância média" : "—"), "#a78bfa", "Quem mais diverge do placar mais votado pelo grupo em cada jogo. A distância é medida por |(palH−palA) − (topH−topA)|, onde topH×topA é o placar mais chutado. Mínimo 3 jogos apostados.");
